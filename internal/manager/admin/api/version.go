...
// AdminVersionAPI manages client binary version configuration and rollout.
type AdminVersionAPI struct {
	store   *store.Store
	gateway *gateway.Gateway
	cache   *cache.Cache
}

func NewAdminVersionAPI(s *store.Store, gw *gateway.Gateway, c *cache.Cache) *AdminVersionAPI {
	return &AdminVersionAPI{
		store:   s,
		gateway: gw,
		cache:   c,
	}
...
func (v *AdminVersionAPI) AdminVersionHandler(w http.ResponseWriter, r *http.Request) {
	switch r.Method {
	case http.MethodGet:
		v.getVersion(w, r)
...
func (v *AdminVersionAPI) getVersion(w http.ResponseWriter, _ *http.Request) {
...
func (v *AdminVersionAPI) setVersion(w http.ResponseWriter, r *http.Request) {
...
// AdminVersionTagsHandler returns recent release tags from GitHub.
// GET /v1/admin/version/tags
func (v *AdminVersionAPI) AdminVersionTagsHandler(w http.ResponseWriter, r *http.Request) {
